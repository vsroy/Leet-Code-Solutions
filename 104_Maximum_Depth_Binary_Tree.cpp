...
int FindMaxDepth(TreeNode* head)
{
	if (head == NULL)
		return 0;
	else
	{
		int lDepth = FindMaxDepth(head->left);
		int rDepth = FindMaxDepth(head->right);
		if (lDepth > rDepth)
			return lDepth + 1;
		else
			return rDepth + 1;
	}
		//return 1 + FindMaxDepth(std::max(head->left, head->right));
}
...
int main()
{
	...
	std::cout << "Max depth of the Bianry Tree is " << FindMaxDepth(head);
